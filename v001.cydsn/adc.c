...
#define THRESHOLD_MV 		(30)        /* minium mv between peaks */
#define MAX_PEAKS 			(705)       /* max peaks that we can store */
#define TRACK_TIMEOUT 		(3000)       /* 500ms between peaks */
#define MIN_POSITIVE_PEAK 	(20)        /* minimum positive value for first peak in ADC units */
#define MIN_NEGATIVE_PEAK 	(-20)       /* minimum negative value for first peak in ADC units */


/* for unit tests */
#define MV_PEAK_MIN 		(100)       /* unit test generates peaks of minumum mV of this value */
#define MV_PEAK_MAX 		(200)       /* unit test generates peaks of maximum mV of this value */
...
{
    bool started:1;
    int16_t peak_idx:15;           // current peak indeax
    uint32_t samples;
    uint32_t lp_tu;             // last peak timestamp in units
...
void end_track(track_t *t)
{
    if (t->peak_idx > 0)
    {
        vLog(xDebug, "detected %i peaks in %i samples\n", t->peak_idx+1, t->samples);
#ifdef DEBUG_PEAKS
        adc_info();
        vLog(xDebug, "adc stopped, cnt_free: %i\n", cnt_free);
        for(int i = 0; i < t->peak_idx; i++)
        {
            xprintf("|%3i: %4i ", t->peaks[i].ts, ADC_SAR_CountsTo_mVolts(Ch_0, t->peaks[i].v));
            if ((i+1) % 16 == 0)
                xputc('\n');
        }
#endif        
...
//        ADC_SAR_CountsTo_mVolts(Ch_Temperature, ADCCountsCorrected)
        ...
        adc_values++;       
        if (trk.started)
            process_sample(&trk, 0xffff, result[Ch_0]);
...
//#if 0
    if (trk.lp_tu != 0 && TimerExpired(TRACK_TIMEOUT+tu2ms(trk.lp_tu)))
    {
        xputc(']');
        vLog(xDebug, "ts_diff: %li, trk.lp_tu = %li (%lims) trk.idx = %li, trk.started: %i\n", GetTick() - (TRACK_TIMEOUT+tu2ms(trk.lp_tu)), trk.lp_tu, tu2ms(trk.lp_tu), trk.peak_idx, trk.started);
        ADC_SAR_StopConvert();
        LPComp_Start(); 
        comp_ts = 0;
        end_track(&trk);
...
void print_adc_stats()
{
    static uint32_t hs_ts = 0;
    static uint8_t  last_display = 0;
    static int adc_min = 0xffff;
    static int adc_max = 0-0xffff;

    if (!TimerExpired(hs_ts))
        return;

    static char buf[256];
    int len = 0;
    
    if (result[Ch_0] < adc_min)
        adc_min = result[Ch_0];
        
    if (result[Ch_0] > adc_max)
        adc_max = result[Ch_0];
    

    int avg = (int)(adc_sum/adc_values);
    len += xsnprintf(buf+len, sizeof(buf)-1-len, "min: %i (%imV), max: %i(%imV), current: %i(%imV), avg: %i(%imV), processed values: %i, reading card: %s",
                    adc_min, ADC_SAR_CountsTo_mVolts(Ch_0, adc_min),
                    adc_max, ADC_SAR_CountsTo_mVolts(Ch_0, adc_max),
                    result[Ch_0], ADC_SAR_CountsTo_mVolts(Ch_0, result[Ch_0]),
                    avg, ADC_SAR_CountsTo_mVolts(Ch_0, avg), 
                    trk.samples,
                    trk.started ? "yes" : "no");
    ...
    bool before = show_timestamps(false);
    len+=print_time();
#endif    
    xprintf("%s", buf);
    ...
    if (last_display > len)
    {
        for (int i = len; i < last_display; i++)
            xputc(' ');
        len = last_display;
    }

     hs_ts = GetTick() + STATS_REFRESH;
        for (int i = 0; i < len; i++)
            xputc('\b');
